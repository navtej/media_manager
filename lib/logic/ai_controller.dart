      ...
      while (!_isDisposed) {
        ...
        final List<Map<String, dynamic>> taskData = currentBatch.map((v) {
          ...
          final Map<String, dynamic> raw =
              (meta['raw'] as Map?)?.cast<String, dynamic>() ?? {};
          final Map<String, dynamic> format =
              (raw['format'] as Map?)?.cast<String, dynamic>() ?? {};
          final Map<String, dynamic> tags =
              (format['tags'] as Map?)?.cast<String, dynamic>() ?? {};
          ...
          final extractedHashtags = hashtagRegex
              .allMatches(promptText)
              .map((m) => m.group(1)!)
              .take(7)
              .toList();
        ...
        for (final res in (results as List)) {
          ...
          final extractedHashtags =
              (res['extractedHashtags'] as List?)?.cast<String>() ?? [];
...
Future<List<Map<String, dynamic>>> _aiIsolateWorker(
  Map<String, dynamic> args,
) async {
  final List<Map<String, dynamic>> tasks = (args['tasks'] as List)
  ...
  for (final task in tasks) {
    ...
    final extractedHashtags =
        (task['extractedHashtags'] as List?)?.cast<String>() ?? <String>[];

    try {
      final suggestions = await NaturalLanguageService.extractTagsStatic(
        prompt,
      );
      results.add({
        'id': id,
